                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSMM ought to be enough for anybody." - Bill Gates, 2017
    ui->zSMMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSMMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSMM->setEnabled(false);
        ui->pushButtonMintzSMM->setToolTip(tr("zSMM is currently disabled due to maintenance."));

        ui->pushButtonSpendzSMM->setEnabled(false);
        ui->pushButtonSpendzSMM->setToolTip(tr("zSMM is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSMMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSMM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSMM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSMM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSMM in ") +
...
void PrivacyDialog::on_pushButtonSpendzSMM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSMM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSMM
        sendzSMM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSMM
    sendzSMM();
}

void PrivacyDialog::on_pushButtonZSmmControl_clicked()
{
    ZSmmControlDialog* zSmmControl = new ZSmmControlDialog(this);
    zSmmControl->setModel(walletModel);
    zSmmControl->exec();
}

void PrivacyDialog::setZSmmControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSmmSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSMM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Smm Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSMMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSMM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SMM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSMM</b>";
    ...
    // use mints from zSmm selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSmmControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSmmControlDialog::GetSelectedMints();
    }
    ...
    // Clear zsmm selector in case it was used
    ZSmmControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSmm Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Smm, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSmm Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSMM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSMM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSMM "));
    ui->labelzSMMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSMMSyncStatus->setVisible(fShow);
}
