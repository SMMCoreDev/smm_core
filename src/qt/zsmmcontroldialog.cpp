...
#include "ui_zsmmcontroldialog.h"
...
std::list<std::string> ZSmmControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSmmControlDialog::listMints;

ZSmmControlDialog::ZSmmControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSmmControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSmmControlDialog::~ZSmmControlDialog()
{
    delete ui;
}

void ZSmmControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSmmControlDialog::updateList()
{
...
void ZSmmControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSmmControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSmm_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSmmControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSmmControlDialog::GetSelectedMints()
{
...
void ZSmmControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
