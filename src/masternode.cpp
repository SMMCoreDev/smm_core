...
#define COLLATERAL_HEIGHT_INCREASE	50000	//RELEASE

#define MASTERNODE_COLLATERAL_LOW (1000 * COIN)
#define MASTERNODE_COLLATERAL_HIGH (1500 * COIN)

CAmount CMasternodeBroadcast::GetMasternodeCollateralToActivate(int nHeight)
{
	if(nHeight >= COLLATERAL_HEIGHT_INCREASE)
...
{
	return GetMasternodeCollateralToActivate(nHeight);
}
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, bool fOfsmm)
{
    CTxIn txin;
    ...
    if (!fOfsmm && !masternodeSync.IsBlockchainSynced()) {
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 10000 SMM tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 SMM tx -> 1 confirmation
