...
#if defined(NDEBUG)
#error "SMM cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 6 * 60 * 60; //6 hours RELEASE
...
int GetHeight()
{
	// non-blocking version to avoid deadlocks
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSmm spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zSmm spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zSmm spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 200000 * COIN;  //premine 200,000
        ...
        nSubsidy = 3 * COIN;
    } else if(nHeight >= 75000 && nHeight < 90000) {
        nSubsidy = 3.5 * COIN;
    } else if(nHeight >= 90000 && nHeight < 120000) {
        nSubsidy = 4 * COIN;
    } else if(nHeight >= 120000 && nHeight < 170000) {
        nSubsidy = 3.5 * COIN;
    } else if(nHeight >= 170000 && nHeight < 250000) {
        nSubsidy = 3 * COIN;
        ...
        int nMul = (nHeight-1000000)/350000;
        if(nMul < 32)
			nSubsidy = (0.25 / pow(2,nMul)) * COIN;
		else
			nSubsidy = 0.0001; //down to zero
    } else {
        nSubsidy = 0.25 * COIN;  //everything else
    }
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK()) {
          ...
          ret = blockValue  / 100 * 75; //75% for MN
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Smm
...
{
    RenameThread("smm-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSMMMinted()
{
...
void RecalculateZSMMSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSMM supply
...
bool RecalculateSMMSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zSmm with serial %s is already in the block %d\n",
    ...
    //LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSmmSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SMM to zSMM
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
		...
		//LogPrintf("CheckBlock: value of stake input is %s\n", FormatMoney(txPrev.vout[block.vtx[1].vin[0].prevout.n].nValue));
		//LogPrintf("CheckBlock: minimal stake is %s\n", FormatMoney(Params().StakeInputMinimal()));
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSmm spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSmm serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSmm mints and %d zSmm spends\n", __func__, nMints, nSpends);
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                // locate a header
                unsigned char buf[MESSAGE_STSMM_SIZE];
                blkdat.FindByte(Params().MessageStart()[0]);
                nRewind = blkdat.GetPos() + 1;
                blkdat >> FLATDATA(buf);
                if (memcmp(buf, Params().MessageStart(), MESSAGE_STSMM_SIZE))
                    continue;
                // read size
                blkdat >> nSize;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Smm: We use certain sporks during IBD, so check to see if they are
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_STSMM_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INVALID MESSAGESTSMM %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
            fOk = false;
            break;
        }
